...
class WordpressSaleOrder(orm.Model):
    ...
    def _function_get_prime_manual_information(
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            # Amazon Prime:
            if ('Amazon Shipment: Service Level NextDay' in wp_order or
                'Service Level NextDay Prime Premium Order' in wp_order or
                'Amazon Shipment: Service Level Standard Prime' in wp_order or
                'Amazon Shipment: Service Level SecondDay Prime Premium Order'
                    in wp_order):
                res[order.id] = 'prime'
            # Manual delivery:
            elif order.marketplace == 'WP' and not order.shipping_total:
                res[order.id] = 'manual'  # todo change condition 'manual'
            # Normal delivery:
            else:
                res[order.id] = 'normal'
        return res
    ...
    _columns = {
        ...
        'shipping_total': fields.float('Totale trasporto', digits=(10, 2)),
        'pricelist_shipping_total': fields.float(
            'Spedizione da listino', digits=(10, 2)),
...
class WordpressSaleOrderLine(orm.Model):
    ...
    _columns = {
        'date_order': fields.related(
            'order_id', 'date_order', string='Data', type='date', store=True),
